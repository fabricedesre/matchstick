...
//
function getYoutubeVideo(videoId, successCallback, errorCallback) {
  var query = 'http://www.youtube.com/get_video_info?&video_id=' + videoId;

  log('About to get info from ' + query);

  var request = new XMLHttpRequest({ mozSystem: true });
  request.open('GET', query);
  request.onerror = function() {
    log('error while querying', query);
    errorCallback();
  };
  request.onload = function() {
    if (request.status !== 200) {
      log('Youtube query return status', request.status, query);
      errorCallback();
      return;
    }

    try {
      log('got ' + request.responseText);
      var info = parseYoutubeVideoInfo(request.responseText);
    }
    catch (e) {
      log('error parsing youtube query response:', e);
      errorCallback();
      return;
    }
  ...
  function parseYoutubeVideoInfo(response) {
    ...
    // If there are adaptative stream formats, add them.
    /*var adaptativeStreams = params.adaptive_fmts;
    if (adaptativeStreams) {
      var l = streams.length;
      var adapt = adaptativeStreams.split(',');
        for (var i = 0, n = adaptativeStreams.length; i < n; i++) {
          streams[i + l] = extractParameters(adaptativeStreams[i]);
      }
    }*/
    ...
    for (var i = 0; i < streams.length; i++) {
      log(JSON.stringify(streams[i]));
    }

    var bestStream = streams[0];
    log('Choosen stream: ' + JSON.stringify(bestStream));
